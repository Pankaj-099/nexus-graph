...
import {
  GitBranch, LogOut, Plus, Network, Layers,
  Trash2, ExternalLink, Search, Clock,
  Activity, Code, Share2, Globe
} from 'lucide-react'
...
export const Dashboard = () => {
  ...
  const { projects, setProjects, addProject, updateProject, removeProject, isLoading, setLoading } = useProjectStore()
  const [showCreate, setShowCreate] = useState(false)
  const [search, setSearch] = useState('')
  const [deleting, setDeleting] = useState<number | null>(null)
  const [shareProject, setShareProject] = useState<Project | null>(null)
  const [form, setForm] = useState<CreateProjectPayload>({
    name: '', description: '', color: '#6366f1', is_public: false,
  })
    ...
    <div className="min-h-screen bg-slate-950">
      ...
      <nav className="border-b border-slate-800 bg-slate-900/80 backdrop-blur-md sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-6 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-slate-600 text-xs hidden sm:block">@{user?.username}</span>
      ...
      <main className="max-w-7xl mx-auto px-6 py-10">
        ...
        <div className="flex items-start justify-between mb-8 gap-4">
          <div>
            ...
            <p className="text-slate-400 text-sm">
              {projects.length} project{projects.length !== 1 ? 's' : ''} · Build and explore your relationship graphs
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
          {[
            ...
            { label: 'Public', value: projects.filter((p) => p.is_public).length, icon: Globe, color: 'text-emerald-400', bg: 'bg-emerald-500/10' },
          ].map((stat) => (
        ...
        {isLoading ? (
          ...
          <Card className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-slate-400 text-sm mb-6 max-w-sm">
              {search ? 'Try a different search term' : 'Create your first project to start building relationship graphs'}
            </p>
            {!search && (
              <Button onClick={() => setShowCreate(true)}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {filtered.map((project) => (
              <Card
                key={project.id}
                className="group hover:border-slate-600 cursor-pointer transition-all duration-200 hover:shadow-lg hover:shadow-black/20"
                ...
                <div className="flex items-start justify-between mb-3">
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all">
                    <button
                      ...
                      className="text-slate-500 hover:text-indigo-400 transition-colors p-1.5 rounded-lg hover:bg-indigo-400/10"
                    >
                      <Share2 size={13} />
                    </button>
                    <button
                    ...
                    >
                      {deleting === project.id ? <Loader size="sm" /> : <Trash2 size={13} />}
                    </button>
                  </div>
                </div>
                {project.description && (
      ...
      <Modal isOpen={showCreate} onClose={() => setShowCreate(false)} title="Create New Project">
        <form onSubmit={handleCreate} className="space-y-4">
          <Input
            label="Project name"
            placeholder="e.g. Company Network, Skills Graph"
            required
            value={form.name}
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <textarea
              ...
              className="w-full bg-slate-800/60 border border-slate-700 rounded-lg text-slate-100 placeholder-slate-500 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 resize-none"
              rows={3}
            />
          </div>
